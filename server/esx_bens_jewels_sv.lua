...
RegisterServerEvent('esx_bens_jewels:toofar')
AddEventHandler('esx_bens_jewels:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_bens_jewels:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('esx_bens_jewels:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_bens_jewels:endrob')
AddEventHandler('esx_bens_jewels:endrob', function(robb)
	local source = source
	local xPlayers = ESX.GetPlayers()
	rob = false
	for i=1, #xPlayers, 1 do
	...
	if(robbers[source])then
		TriggerClientEvent('esx_bens_jewels:robberycomplete', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_ended') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_bens_jewels:rob')
AddEventHandler('esx_bens_jewels:rob', function(robb)
	...
	if Stores[robb] then
		...
		if (os.time() - store.lastrobbed) < 600 and store.lastrobbed ~= 0 then
            ...
            TriggerClientEvent('esx_bens_jewels:togliblip', source)
			...
			if(cops >= Config.RequiredCopsRob)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('esx_bens_jewels:setblip', xPlayers[i], Stores[robb].position)
					end
				end
			    ...
			    TriggerClientEvent('esx_bens_jewels:currentlyrobbing', source, robb)
                CancelEvent()
...
RegisterServerEvent('esx_bens_jewels:gioielli1')
AddEventHandler('esx_bens_jewels:gioielli1', function()
